...
const WikiSubNav = ({ article, setArticle }: WikiSubNavProps) => {
  const btns: { [key: string]: string[] } = {
    controller: ["Move", "Menu", "Action", "Interact"],
    t2mon: ["Stats", "Types", "Storage", "T2mon"],
    spell: ["Effect", "Mana", "Types", "Spell"],
    item: ["Effect", "Shop", "Use", "Item"],
    fight: ["Start", "Catch", "Order", "Outcome"],
  };
    ...
    sections.forEach((r) => {
      result.push(
        <Route exact path={`*/${r}`} key={result.length}>
          <>
            {btns[r].map((btn: string, index: number) => {
              return (
                <NavBtnArticle
